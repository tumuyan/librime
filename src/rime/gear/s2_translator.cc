...
class S2Syllabifier : public PhraseSyllabifier {
 public:
  S2Syllabifier(S2Translator* translator,
                    Corrector* corrector,
                    const string& input,
                    size_t start)
...
class S2Translation : public Translation {
 public:
  S2Translation(S2Translator* translator,
                    Corrector* corrector,
                    Poet* poet,
                    const string& input,
                    size_t start)
      : translator_(translator),
        poet_(poet),
        start_(start),
        syllabifier_(New<S2Syllabifier>(
...
// S2Translator implementation

S2Translator::S2Translator(const Ticket& ticket)
    : Translator(ticket),
      Memory(ticket),
...
an<Translation> S2Translator::Query(const string& input,
  ...
  auto result = New<S2Translation>(this,
                                       corrector_.get(),
                                       poet_.get(),
                                       input,
                                       segment.start);
...
string S2Translator::FormatPreedit(const string& preedit) {
  string result = preedit;
  preedit_formatter_.Apply(&result);
  return result;
}

string S2Translator::Spell(const Code& code) {
  string result;
...
string S2Translator::GetPrecedingText(size_t start) const {
...
bool S2Translator::Memorize(const CommitEntry& commit_entry) {
...
// S2Syllabifier implementation

Spans S2Syllabifier::Syllabify(const Phrase* phrase) {
  Spans result;
...
size_t S2Syllabifier::BuildSyllableGraph(Prism& prism) {
  return (size_t)syllabifier_.BuildSyllableGraph(input_,
                                                 prism,
                                                 &syllable_graph_);
}

bool S2Syllabifier::IsCandidateCorrection(const rime::Phrase &cand) const {
...
string S2Syllabifier::GetPreeditString(const Phrase& cand) const {
...
string S2Syllabifier::GetOriginalSpelling(const Phrase& cand) const {
  if (translator_ &&
...
// S2Translation implementation

bool S2Translation::Evaluate(Dictionary* dict, UserDictionary* user_dict) {
...
void S2Translation::PrepareCandidate() {
  if (exhausted()) {
    candidate_ = nullptr;
    return;
  }
...
template <class QueryResult>
void S2Translation::EnrollEntries(
...
an<Sentence> S2Translation::MakeSentence(Dictionary* dict,
